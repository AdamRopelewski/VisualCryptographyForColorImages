...
def encode_image_2(
    ...
    def generate_tile(x):
        for y in range(original_height * 3):
            ...
            # If the pixel is black, we paste the corresponding tile based on the first encoded image
...
def encode(image_path, max_threads=4, progress_var=None, cwd=""):
    ...
    dithered_image.save(cwd + output_folder + "/1_dithered_image.png")
    if progress_var:
    ...
    x3_res_dithered_made_of_tiles_image.save(
        cwd + output_folder + "/2_x9_res_dithered_made_of_tiles_image.png"
    )
    if progress_var:
    ...
    encoded_image_1.save(cwd + output_folder + "/3_encoded_image_1.png")
    if progress_var:
    ...
    encoded_image_2.save(cwd + output_folder + "/4_encoded_image_2.png")
    if progress_var:
...
def decode(file_path_1, file_path_2, max_threads=4, progress_var=None, cwd=""):
    ...
    if not os.path.exists(output_folder):
        os.makedirs(output_folder)
    decoded_image.save(cwd + output_folder + "/5_decoded_image_xor_and.png")
    if progress_var:
    ...
    decoded_image_restored_colors.save(
        cwd + output_folder + "/6_decoded_image_restored_colors.png"
    )
